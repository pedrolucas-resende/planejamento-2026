...
-- Novas CTE
  ...
  clustered AS (
  SELECT *,
    NTILE(5) OVER (ORDER BY populacao) AS populacao_percentil
  FROM dados_regionais
  WHERE populacao IS NOT NULL
)


select t.*,  i.qtd_faturada_mes, qtd_produzidas, d.qtd_vendas_0km, d.qtd_vendas_semi, d.qtd_vendas_usada, prod.qtd_internas, qtd_cliente, prod.mec_total, prod.mec_presentes, prod.mec_na_rampa, cl.* except (filial, pais),
...
-- Adiciona informacoes regionais e populacionais
  ...
  CASE
        ...
        WHEN cl.regiao = 'Sudeste' AND (cidade = 'São Paulo')
          THEN 'São Paulo (Capital)'
        WHEN cl.regiao = 'Sudeste' AND (cidade = 'Rio de Janeiro')
          THEN 'Rio de Janeiro (Capital)'
        ELSE cl.regiao
